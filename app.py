...
buy_points = df[df["Entry_Signal"] == 1].copy()
short_points = df[df["Entry_Signal"] == -1].copy()

# Place signal markers slightly away from the candlesticks so they stay visible.
buy_points["Marker_Y"] = buy_points["Low"] * 0.975
short_points["Marker_Y"] = short_points["High"] * 1.025

# ============================================================
# Price chart
# The title is rendered by Streamlit instead of Plotly. This permanently
# prevents the chart title from overlapping the horizontal legend.
# ============================================================
st.subheader(f"{ticker} 股價與交易訊號")
st.caption("亮綠色 ▲ 代表 BUY；亮紫色 ▼ 代表 SHORT。")
...
price_fig.add_trace(
    go.Candlestick(
        ...
        close=df["Close"],
        name="Price",
        increasing_line_color="#00A896",
        decreasing_line_color="#E63946",
        increasing_fillcolor="#7AE7D4",
        decreasing_fillcolor="#FF8FA3",
    )
)
...
price_fig.add_trace(
    go.Scatter(
        ...
        hovertemplate=(
            ...
            "<br>Volatility=%{customdata[1]:.2%}"
            "<br>Regime=%{customdata[2]}<extra></extra>"
        ),
    )
)
...
# ============================================================
# Volatility chart
# Rendered as a separate chart so its title and legend cannot collide with
# the price chart on narrower browser windows.
# ============================================================
st.subheader(f"{volatility_window} 日年化波動度")
...
volatility_fig.add_trace(
    go.Scatter(
        ...
        fill="tozeroy",
        fillcolor="rgba(0,166,251,0.12)",
    )
)

volatility_fig.add_hrect(
    y0=0,
    y1=low_threshold,
...
volatility_fig.add_hrect(
    y0=high_threshold,
    y1=max(float(df["Volatility"].max()) * 1.08, high_threshold * 1.15),
    fillcolor="rgba(231, 76, 60, 0.10)",
    line_width=0,
    layer="below",
)

volatility_fig.add_hline(
    y=low_threshold,
    line_dash="dash",
    line_color="#1B9E4B",
    annotation_text=f"低波動門檻 {low_threshold:.1%}",
...
volatility_fig.add_hline(
    ...
    line_dash="dash",
    line_color="#D62728",
    annotation_text=f"高波動門檻 {high_threshold:.1%}",
...
volatility_fig.update_layout(
    ...
    yaxis_title="年化波動度",
    yaxis_tickformat=".0%",
    margin=dict(l=55, r=30, t=25, b=45),
)
